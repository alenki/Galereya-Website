...
async function Load() {
    ...
    await stores.getStores().then(stores => {
        ...
        if(service_open == "open"){
            openStore(localStorage.getItem("service_id"));
...
class UI {
    displayStores(stores){
        ...
        var category = localStorage.getItem("category");
        stores.forEach(stores => {
            if(stores.englishCategory == category){
                result += `                            
        <!-- single stores -->
                    <article class="store-block storeButton" id="${stores.id}">
                      ...
                      <div class="store-info storeButton" id="${stores.id}"">
                        <div class="store-name-box storeButton" id="${stores.id}">
                          <h3 class="store-name storeButton" id="${stores.id}">${stores.englishTitle}</h3>
                          <p class="store-caption storeButton" id="${stores.id}">${stores.englishDescription}</p>
                        </div>
                        <div class="storeButton" id="${stores.id}">
                          <span class="store-floor storeButton" id="${stores.id}">${stores.floor} floor</span>
                        </div>
        ...
        await stores.getStores().then(stores => {
            stores.forEach(stores => {
                if(stores.id == storeID) {
                    ...
                    storeName.innerHTML = stores.englishTitle;
                    storeFloor.innerHTML = stores.floor + ' floor';
                    storeAbout.innerHTML = stores.englishDescription;
                }
            });
        })
